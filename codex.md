...
- [x] Observability `0020` schema、pure rule/evaluator/parser、evaluator runtime-status/lease/bootstrap repository、bounded 十規則 `audit_event` source repository、bounded OAuth-report source repository、bounded logout-delivery source repository 與 archive record/envelope crypto contract 已加入 local source 並具負面測試；四個 repository 都尚未接入 Worker/scheduler，logout source 也不改動 delivery/replay；其餘 fan-out/Queue sources、`alert_state`/`security_alert`/`alert_outbox` CAS、Cron、alert Queue/DLQ、Email/admin delivery 與同輪 execution proof 仍不存在，所以 observability 必須維持 `source_present_unverified`，pure crypto 也不能把 `encrypted_r2_archive` 從 `dependency_missing` 升級。
