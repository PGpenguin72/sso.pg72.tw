...
截至 2026-07-18，repository source 包含 SSO Worker、React 帳號中心、D1 migrations `0001`–`0023`、Google/Passkey、可選社群登入、OAuth 2.1 Provider、四級角色、邀請/停權/audit/client 管理、ID-token central `sid` contract、durable global logout ledger/outbox/Queue delivery、Mail Path A introspection prerequisite、Passkey client-mutation step-up、Telegram verified-email enrollment boundary、全域 provider-identity 唯一 ownership、Turnstile-backed public-registration intent、版本化法律同意紀錄、公開新帳號 restricted access 與 abuse-response runbook、預設關閉且 hash-only 的 recovery-code/Passkey replacement flow、observability `0020` schema、pure alert rule/evaluator/parser、尚未接入 Worker entry point/scheduler 的 evaluator run/runtime、alert lifecycle snapshot/CAS 與 bounded audit/OAuth/fan-out/logout/runtime-health/Queue-DLQ source repositories、`0022` evaluator run/source/decision proof ledger、canonical manifest serializer、相容 run repository、state 同批 applied/no-state decision proof integration，以及尚未接入 entry point/scheduler 的九項 source/decision evaluator orchestration，另有 pure audit-archive record/envelope crypto contract、`0021` archive ledger、`0023` R2 evidence forward migration、尚未接入 Worker/Queue/Cron 的 D1 repository、只接受 injected R2/envelope verifier 的 pure create-only writer，以及只接受 externally supplied expected manifest、injected R2 read surface 與 custody opener 的 pure one-object restore verifier，並有使用 `oauth4webapi` 的獨立 test RP。這些 local API 與 Workerd integration 可證明受控 D1 transaction、synthetic 同輪 proof、bounded writer decision 與 source-level restore verification；但 scheduled handler、generated bindings/config、Queue/Email/operator wiring、archive runtime binding、real KEK/custody adapter、isolated restore sink/exercise、external backup 與 remote evidence 都不存在，不能宣稱 production observability 或 encrypted archive 已完成。每個 release candidate 都必須重跑本 repository 的 typecheck、workerd suite、production build 與 test RP protocol gate；本地通過不得寫成遠端已部署。
...
Local source 也包含 pure audit-archive record/envelope crypto contract：canonical record 保存 nullable actor reference/version，且 header、manifest 與 AES-GCM AAD 綁定 `checkpointFromSequence`。`0021_audit_archive.sql` 提供 monotonic source ledger、immutable batch membership、archive-key sentinel schema、同 lease 的 bounded renewal，以及 trigger-coupled D1 finalization/checkpoint/BLOB cleanup；`0023_audit_archive_r2_evidence.sql` 以前向 migration 加入不造證據的 legacy conflict discriminator、current observed conflict evidence，以及成功封存的 immutable object size/stored SHA/readback SHA fence。尚未接線的 request-scoped D1 repository 只接受預先派生的 fingerprint reference，並負責 bounded source prefix、canonical item/parent persistence、dispatch-generation claim、lease-bound envelope read、bounded due/expired/checkpoint work selection、renew/terminal receipt、response-loss idempotency 與 audited dead replay。Pure writer 透過 injected R2 binding surface 與 envelope verifier 執行 create-only PUT、strong-consistent bounded readback、stored/full SHA 與 byte equality、第二次 envelope 驗證和 exact terminal classification，但沒有 entrypoint/config/binding/KEK adapter。Pure restore verifier 則在第一個 injected R2 read 前驗證 externally supplied exact v1 manifest 與 deterministic object key，之後驗證 metadata、size、stored/computed SHA，透過不含 KEK 的 injected custody opener 開啟，並重新 canonicalize、核對及 detach record set。這仍不是完整 encrypted R2 archive implementation。Archive-specific fingerprint derivation、real KEK/custody adapter、writer/restore runtime integration 與 remote proof、isolated restore sink/exercise、Queue/DLQ、Cron redrive、retention proof 與 Cloudflare 之外的 backup 仍未實作，所以 `encrypted_r2_archive` 必須維持 `dependency_missing`。完整 source boundary 見 [`docs/runbooks/alert-observability.md`](./docs/runbooks/alert-observability.md) 與 [`docs/runbooks/audit-archive.md`](./docs/runbooks/audit-archive.md)。
...
- [x] Observability `0020` schema、pure rule/evaluator/parser、尚未接入 Worker entry point/scheduler 的 bounded audit/OAuth/fan-out/logout/runtime-health/Queue-DLQ source repositories、`0022` evaluator run/source/decision proof ledger、canonical manifest、相容 run repository、lifecycle snapshot 與同批 applied/no-state proof、unwired 九來源 evaluator orchestration、archive record/envelope crypto contract、`0021` ledger、`0023` evidence contract、unwired D1 archive repository、pure unwired create-only R2 writer 與 pure one-object restore verifier 已加入 local source 並具負面/競態測試；restore verifier 在任何 R2 read 前驗證 external expected manifest/key，並 revalidate/detach custody opener records。Evaluator orchestrator、所有 observability/archive repository、writer 與 restore verifier 仍未接入 Worker/scheduler，fan-out source 不提供 durable delivery/replay，logout source 不改動 delivery/replay，runtime-health source 不排程 evaluator 或 claim/deliver outbox，Queue source 未新增 binding 或 sampling loop，archive repository/writer/restore verifier 未新增 binding/real KEK-custody adapter/Queue/Cron/runtime wiring。Scheduled handler、generated bindings/config、alert/archive Queue/DLQ、Email/admin delivery、archive fingerprint derivation/KEK custody、writer/restore runtime integration 與 remote proof、isolated restore sink/exercise、external backup 與 production 同輪 execution proof 驗收仍不存在，所以 observability 必須維持 `source_present_unverified`，archive source 也不能把 `encrypted_r2_archive` 從 `dependency_missing` 升級。
...
Local migration `0020` 提供 fail-closed schema 與查詢索引；repository 另有 pure rule/evaluator/parser、尚未接入 Worker entry point/scheduler 的 evaluator runtime-status/lease/bootstrap 與 alert state/incident/outbox CAS repositories、只涵蓋十項 `audit_event` 規則的 bounded source repository、bounded OAuth-report source repository、bounded global fan-out-gap source repository、bounded logout-delivery source repository、bounded global runtime-health source repository、bounded approximate Queue-DLQ source/streak repository，以及 audit-archive crypto contract、`0021` ledger、`0023` evidence contract、unwired D1 archive repository、pure unwired create-only R2 writer 與 pure one-object restore verifier。Fan-out source 只讀 audit/marker gap，不提供 durable general delivery；logout source 只讀 durable D1 delivery/attempt evidence，不改變 delivery runtime；runtime-health source 只在一個 D1 batch 讀取既有 evaluator/bootstrap 與 all-age outbox aggregate，不排程、claim 或 delivery；Queue source 只解析 request-scoped `Queue.metrics()` 並以 D1 CAS 保存 60-second streak，未新增 binding 或 sampling loop；archive repository 只提供 D1 source/lease/persistence/replay contract，writer 只接受 injected R2/envelope-verifier 並執行 bounded decision，restore verifier 只接受 externally supplied expected manifest、injected R2 read surface 與 custody opener，三者都未新增 binding、real KEK/custody adapter、Queue、Cron 或 runtime wiring。`0022_alert_evaluator_run_proof.sql` 另提供 evaluator run/source/decision proof ledger、compatible run repository、validated previous-state/CAS readers 與 local 九來源 evaluator orchestration，但 orchestrator 仍未接入 Worker/scheduled handler。其餘 scheduled handler、generated bindings/config、dedicated alert/archive Queue/DLQ、Email/admin delivery，以及 archive fingerprint derivation/KEK custody、writer/restore runtime integration 與 remote proof、isolated restore sink/exercise、external backup 仍未實作，因此 observability 狀態仍是 `source_present_unverified`，`encrypted_r2_archive` 仍是 `dependency_missing`。詳細邊界見 [`docs/runbooks/alert-observability.md`](./docs/runbooks/alert-observability.md) 與 [`docs/runbooks/audit-archive.md`](./docs/runbooks/audit-archive.md)。
