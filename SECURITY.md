...
The repository's local source now includes the narrowly scoped Mail Path A introspection prerequisite, Passkey step-up for every OAuth client mutation, public-registration prerequisites using Turnstile, versioned legal acceptance and persistent restricted-account access, the global-logout source contract, recovery migration `0019`, observability migration `0020`, pure alert rule/evaluator/parser modules, and the pure audit-archive record/envelope crypto contract. The alert modules and archive crypto are testable source contracts only: they are not wired into the Worker entry point and do not provide the D1 source repository/CAS runtime, evaluator Cron, dedicated alert or archive Queue/DLQ, Email Service adapter, operator API/UI, archive R2 writer/checkpoint/restore, external backup, or same-run execution proof. Observability must therefore remain `source_present_unverified`, while `encrypted_r2_archive` remains `dependency_missing` until the separate `0021` repository and storage contract exists and is exercised.
