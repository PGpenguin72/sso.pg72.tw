...
The repository's local source now includes the narrowly scoped Mail Path A introspection prerequisite, Passkey step-up for every OAuth client mutation, public-registration prerequisites using Turnstile, versioned legal acceptance and persistent restricted-account access, the global-logout source contract, recovery migration `0019`, observability migration `0020`, pure alert rule/evaluator/parser modules, an evaluator runtime-status/lease/bootstrap repository, bounded source slices for ten `audit_event` rules, the OAuth client-report rule, the logout-delivery health rule, and the approximate Queue-DLQ rule, plus the pure audit-archive record/envelope crypto contract. The runtime repository owns initialization, lease acquisition/renewal, terminal success/failure, immutable first-success bootstrap creation, and the exact projection read; the audit repository owns only the bounded audit-rule observations; the OAuth repository owns bounded half-open report cohorts and nullable reporter-provenance evidence; the logout repository owns only independent global and HMAC-client observations over durable `0018` delivery/attempt evidence; the Queue repository owns only strict request-scoped `Queue.metrics()` parsing and 60-second D1 streak persistence for the four closed DLQ dimensions. None of these repositories is imported by the Worker entry point or a scheduler. The logout source does not change delivery, replay, Queue, or Cron guarantees, and the Queue metrics source adds no binding or sampling loop. The alert modules and archive crypto still do not provide the remaining fan-out sources, `alert_state`/`security_alert`/`alert_outbox` CAS, evaluator Cron, dedicated alert or archive Queue/DLQ, Email Service adapter, operator API/UI, archive R2 writer/checkpoint/restore, external backup, or same-run execution proof. Observability must therefore remain `source_present_unverified`, while `encrypted_r2_archive` remains `dependency_missing` until the separate `0021` repository and storage contract exists and is exercised.

Production has none of migrations `0013` through `0020` or this Worker version; `pgid-mail-introspect` has not been provisioned, no public-registration or logout/alert Queue bindings have been configured, `RECOVERY_MODE` remains disabled, no remote D1 operation was performed, and the mail VPS and production RP receivers have not been cut over. These local results must not be represented as production behavior.
