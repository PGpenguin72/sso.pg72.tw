...
- [x] Observability `0020` schema、pure rule/evaluator/parser、尚未接入 Worker entry point/scheduler 的 evaluator runtime-status/lease/bootstrap 與 alert state/incident/outbox CAS repositories、bounded 十規則 `audit_event` source repository、bounded OAuth-report source repository、bounded global fan-out-gap source repository、bounded logout-delivery source repository、bounded global runtime-health source repository、bounded approximate Queue-DLQ source/streak repository、archive record/envelope crypto contract、`0021` ledger 與 unwired D1 archive repository 已加入 local source 並具負面/競態測試；九個 repository 都尚未接入 Worker/scheduler，fan-out source 不提供 durable delivery/replay，logout source 不改動 delivery/replay，runtime-health source 不排程 evaluator 或 claim/deliver outbox，Queue source 未新增 binding 或 sampling loop，archive repository 也未新增 Queue/R2/runtime wiring；`0022_alert_evaluator_run_proof.sql`、evaluator run/source/decision proof ledger、validated previous-state/CAS readers、其餘 Cron、alert/archive Queue/DLQ、Email/admin delivery、archive fingerprint derivation/KEK custody、R2 writer/restore/external backup 與同輪 execution proof 仍不存在，所以 observability 必須維持 `source_present_unverified`，archive source 也不能把 `encrypted_r2_archive` 從 `dependency_missing` 升級。
