# Audit archive schema boundary

> Status: local D1 ledger source only. Migration `0021_audit_archive.sql`
> does not read a KEK, write or restore R2, publish or consume Queue messages,
> schedule Cron work, delete retained objects, or provide an operator endpoint.
...
`quick_check` and foreign-key integrity.

## Remaining gates

Schema presence does not satisfy encrypted archive continuity. The dependency
must remain `dependency_missing` until a separately reviewed slice adds all of
the following and executes their proof in the same run:

- disabled-by-default repository and KEK continuity handling;
