...
`0022_alert_evaluator_run_proof.sql` 以三張 additive redacted table 固定 logical Cron occurrence、evaluator lease/run lifecycle、closed 九項 source proof 與 source-attributed decision proof；run insert/renewal 與 runtime terminal transition 由 trigger 在同一 SQLite statement 反向耦合，CAS miss 會 rollback，且直接 legacy evaluator acquire/renew/idle/terminal update 在 `0022` 後 fail closed。Seal 必須已有 exact 九項 source row；healthy terminal 必須 sealed、九項 complete、decision count 相符、沒有 suppressed-partial，並晚於所有 proof write。這仍只有 local schema/negative test；相容 repository/state batch/Cron 尚未加入本 checkpoint，production migration record 仍只到 `0012`，不得先套 `0022` 再部署舊 runtime 呼叫路徑。
...
- [x] Observability `0020` schema、pure rule/evaluator/parser、尚未接入 Worker entry point/scheduler 的 evaluator runtime-status/lease/bootstrap 與 alert state/incident/outbox CAS repositories、bounded 十規則 `audit_event` source repository、bounded OAuth-report source repository、bounded global fan-out-gap source repository、bounded logout-delivery source repository、bounded approximate Queue-DLQ source/streak repository、`0022` evaluator run/source/decision proof schema foundation、archive record/envelope crypto contract、`0021` ledger 與 unwired D1 archive repository 已加入 local source 並具負面/競態測試；`0022` 相容 repository/state 同批 proof/Cron 尚未加入，既有八個 repository 也都尚未接入 Worker/scheduler，fan-out source 不提供 durable delivery/replay，logout source 不改動 delivery/replay，Queue source 未新增 binding 或 sampling loop，archive repository 也未新增 Queue/R2/runtime wiring；其餘 Cron、alert/archive Queue/DLQ、Email/admin delivery、archive fingerprint derivation/KEK custody、R2 writer/restore/external backup 與完整同輪 execution proof 仍不存在，所以 observability 必須維持 `source_present_unverified`，archive source 也不能把 `encrypted_r2_archive` 從 `dependency_missing` 升級。
