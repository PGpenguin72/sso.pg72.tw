...
- [x] Observability `0020` schema、pure rule/evaluator/parser、evaluator runtime-status/lease/bootstrap repository 與 archive record/envelope crypto contract 已加入 local source 並具負面測試；runtime repository 尚未接入 Worker/scheduler，完整 D1 audit/other metric-source aggregation、`alert_state`/`security_alert`/`alert_outbox` CAS、Cron、alert Queue/DLQ、Email/admin delivery 與同輪 execution proof 仍不存在，所以 observability 必須維持 `source_present_unverified`，pure crypto 也不能把 `encrypted_r2_archive` 從 `dependency_missing` 升級。
...
Local migration `0020` 提供 fail-closed schema 與查詢索引；repository 另有 pure rule/evaluator/parser、evaluator runtime-status/lease/bootstrap repository 和 audit-archive crypto contract，但沒有把它們接入 Worker runtime。完整 D1 audit/other metric-source aggregation、`alert_state`/`security_alert`/`alert_outbox` CAS、observability Cron、dedicated Queue/DLQ、Email/admin delivery、same-run proof 或 `0021` R2 archive/restore/external backup 仍不存在時，observability 狀態必須維持 `source_present_unverified`，`encrypted_r2_archive` 仍是 `dependency_missing`。詳細邊界見 [`docs/runbooks/alert-observability.md`](./docs/runbooks/alert-observability.md)。
