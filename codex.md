...
- [x] Repository source 已有 `pnpm security:check` 自動化 gate：type-aware Worker Promise SAST、checksum-pinned Gitleaks 完整 history、明確枚舉 tracked/untracked/ignored sensitive path 且使用 normalized assignment key、exact raw-path/key/value fixture triple 與 unsafe-path hash diagnostics 的 bounded redacted working-tree scan、GitHub Action SHA、code-owned workflow/job/step exact environment scope 與遞迴 workflow/package-script allowlist、Wrangler source/generated typed binding/resource exact contract、exact advisory reconciliation、production Worker dry-run artifact/source-map/private-path/text+binary secret/size scan，以及 dependency/license inventory；每個 release candidate 仍必須先安裝 pinned tools、實際跑完並保存結果。
...
- Required checksum-pinned Gitleaks history、captured Secretlint，加上不受 gitignore 影響的 tracked/untracked/ignored-sensitive-path bounded scanner；assignment parser 先剝除 bounded declaration/object-key prefix，再將 camelCase／separator 正規化為 case-insensitive key token，source fixture 與 reviewed generated enum/metadata/required-config fallback sentinel allowance 只接受 exact raw path/normalized key/complete value triple，不以 test/example/placeholder substring 放行。production dry-run artifact 共用同一 text/binary secret family，並檢查 private-path/source-map/unexpected-file/symlink/size；diagnostic path 先 normalize，sensitive/secret-bearing/absolute-outside/control-character path 只輸出短 SHA-256 identifier。
- GitHub Actions immutable SHA/least permissions/concurrency/retention、code-owned workflow/job/step exact environment key/value/expression scope（含 inheritance、inline assignment 與 `GITHUB_ENV`/`GITHUB_PATH` mutation 拒絕）；`CLOUDFLARE_*`、legacy `CF_*`、`WRANGLER_*` 不可由 policy 放行。另含 reachable package/local-script exact allowlist、Preview actor/ref guard，以及 Wrangler JSON Schema 與 source/generated D1/Queue/DLQ/Rate Limit/route/assets typed target exact contract。
