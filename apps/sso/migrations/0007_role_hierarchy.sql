...
--
-- This rebuild runs after 0006_release_consumed_invitations and must keep
-- every property that migration established: consumed_by_user_id cascades
-- with the consuming account (ON DELETE CASCADE), a consumed invitation
-- always records who consumed it (the consumed_at/consumed_by_user_id
-- CHECK), and rows orphaned by the pre-0006 SET NULL behaviour stay dropped
-- (the WHERE clause below is a defensive repeat of the 0006 cleanup).
