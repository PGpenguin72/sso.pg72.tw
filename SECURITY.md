...
The repository's local source now includes the narrowly scoped Mail Path A introspection prerequisite, Passkey step-up for every OAuth client mutation, public-registration prerequisites using Turnstile, versioned legal acceptance and persistent restricted-account access, the global-logout source contract, recovery migration `0019`, observability migration `0020`, pure alert rule/evaluator/parser modules, a bounded source slice for ten `audit_event` rules, and the pure audit-archive record/envelope crypto contract. The alert modules and archive crypto are testable source contracts only: they are not wired into the Worker entry point and do not provide the remaining OAuth-report/fan-out/logout/runtime/Queue sources, D1 state/incident CAS runtime, evaluator Cron, dedicated alert or archive Queue/DLQ, Email Service adapter, operator API/UI, archive R2 writer/checkpoint/restore, external backup, or same-run execution proof. Observability must therefore remain `source_present_unverified`, while `encrypted_r2_archive` remains `dependency_missing` until the separate `0021` repository and storage contract exists and is exercised.
...
- independently review the integrated local recovery and release-automation source/proofs, finalize the Worker artifact identity only after all runtime inputs are frozen, and complete and independently review the observability repository/Cron/delivery proof plus the `0021` encrypted R2 writer/checkpoint/restore and external-backup path; the local `0020` schema, pure evaluator/parser, and archive crypto contract do not complete this gate, and only then may a clean synthetic run be recorded without treating it as remote evidence;
