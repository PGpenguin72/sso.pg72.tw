...
- `ci.yml`、`dast-preview.yml` exact file set 與 raw LF bytes 由 code-owned SHA-256 固定，CRLF 或任何 action `uses/with`、checkout input、job/step control、run/env、upload path/retention/hidden/comment 變更都在 YAML/deep checks 前 fail；GitHub Actions immutable SHA/least permissions/concurrency/retention、code-owned workflow/job/step exact run string map、reachable package-script exact name/complete-value canonical digest，以及 exact environment key/value/expression scope仍作第二層。policy 不可新增 command/local-script/leaf allowance；release artifact upload 固定為 `.artifacts/release`、missing=error、hidden=false、retention=7。`CLOUDFLARE_*`、legacy `CF_*`、`WRANGLER_*` 不可由 policy 放行。另含 Preview actor/ref guard，以及 Wrangler JSON Schema 與 source/generated D1/Queue/DLQ/Rate Limit/route/assets typed target exact contract。
