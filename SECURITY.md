...
The repository's local source now includes the narrowly scoped Mail Path A introspection prerequisite, Passkey step-up for every OAuth client mutation, public-registration prerequisites using Turnstile, versioned legal acceptance and persistent restricted-account access, the global-logout source contract, recovery migration `0019`, observability migration `0020`, pure alert rule/evaluator/parser modules, evaluator runtime-status/lease/bootstrap and alert-state/incident/outbox compare-and-swap repositories, bounded source slices for ten `audit_event` rules, the OAuth client-report rule, and the global fan-out-gap rule, plus the pure audit-archive record/envelope crypto contract and schema-only archive ledger migration `0021`. The runtime repository owns initialization, lease acquisition/renewal, terminal success/failure through exact compare-and-swap, immutable first-success bootstrap creation through `INSERT ... SELECT`, and the exact projection read; the state repository atomically persists one already-pure evaluator decision with revision fencing, incident state, immutable Email outbox snapshot, canonical payload digest, and explicit applied/conflict/duplicate outcomes; the audit repository owns only the bounded audit-rule observations; the OAuth repository owns bounded half-open report cohorts and nullable reporter-provenance evidence; the fan-out repository owns only a global one-hour half-open audit/marker projection with strict five/fifteen-minute grace cohorts. None of these repositories is imported by the Worker entry point or a scheduler. The fan-out source does not add durable general security-event delivery, a replayer, or Queue wiring. The remaining logout/Queue sources, evaluator Cron, dedicated alert or archive Queue/DLQ, Email Service adapter, operator API/UI, archive R2 writer/bounded restore, external backup, and same-run execution proof remain absent. Observability must therefore remain `source_present_unverified`, while `encrypted_r2_archive` remains `dependency_missing`; the `0021` ledger alone does not make encrypted archive storage operational or verified.

The undeployed local `0020` source also adds sparse single-column indexes over only non-canonical `audit_event.occurred_at` and `oauth_client_report.created_at` rows, then prevents future non-canonical inserts or timestamp updates. Audit, OAuth-report, and fan-out repositories probe the appropriate sparse index with a covering `EXISTS ... LIMIT 1` statement at the start of the same D1 batch as their lexical windows. Any legacy corruption fails closed with a fixed repository error; a healthy empty index permits the bounded window reads. `0021` independently validates each parent audit timestamp by primary key before capture or backfill can allocate a sequence. These are local migration/source contracts, not evidence that production has applied them or that observability is operational.
