...
- versioned D1 migrations through local source `0017`: `0013` normalizes confidential client authentication, `0014` adds Passkey step-up state, `0015` enforces global provider-identity ownership, `0016` adds one-time public-registration intents plus immutable legal-acceptance history, and `0017` adds persistent restricted-account access; the latest production record remains applied through `0012` until the owner verifies and applies the pending migrations in order;
...
3. Run the provider-identity duplicate preflight in `codex.md` §0.1, apply the reviewed pending migrations in numeric order (`0013`, `0014`, `0015`, `0016`, then `0017`), and deploy the verified Worker source with `PASSKEY_STEP_UP_MAX_AGE_SECONDS=600` plus Rate Limiting namespaces `1004` and `1005` bound as configured. Migrations `0016` and `0017` are required by this Worker schema even while production remains invite-only; applying them does not authorize changing `REGISTRATION_MODE`.
