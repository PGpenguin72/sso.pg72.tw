...
The repository's local source now includes the narrowly scoped Mail Path A introspection prerequisite, Passkey step-up for every OAuth client mutation, public-registration prerequisites using Turnstile, versioned legal acceptance and persistent restricted-account access, the global-logout source contract, recovery migration `0019`, observability migration `0020`, pure alert rule/evaluator/parser modules, an evaluator runtime-status/lease/bootstrap repository, bounded source slices for ten `audit_event` rules and the OAuth client-report rule, and the pure audit-archive record/envelope crypto contract. The runtime repository owns initialization, lease acquisition/renewal, terminal success/failure, immutable first-success bootstrap creation, and the exact projection read; the audit repository owns only the bounded audit-rule observations; the OAuth repository owns bounded half-open report cohorts and nullable reporter-provenance evidence. None of these repositories is imported by the Worker entry point or a scheduler. The alert modules and archive crypto still do not provide the remaining fan-out/logout/Queue sources, `alert_state`/`security_alert`/`alert_outbox` CAS, evaluator Cron, dedicated alert or archive Queue/DLQ, Email Service adapter, operator API/UI, archive R2 writer/checkpoint/restore, external backup, or same-run execution proof. Observability must therefore remain `source_present_unverified`, while `encrypted_r2_archive` remains `dependency_missing` until the separate `0021` repository and storage contract exists and is exercised.
