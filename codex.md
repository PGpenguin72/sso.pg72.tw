...
截至 2026-07-18，repository source 包含 SSO Worker、React 帳號中心、D1 migrations `0001`–`0023`、Google/Passkey、可選社群登入、OAuth 2.1 Provider、四級角色、邀請/停權/audit/client 管理、ID-token central `sid` contract、durable global logout ledger/outbox/Queue delivery、Mail Path A introspection prerequisite、Passkey client-mutation step-up、Telegram verified-email enrollment boundary、全域 provider-identity 唯一 ownership、Turnstile-backed public-registration intent、版本化法律同意紀錄、公開新帳號 restricted access 與 abuse-response runbook、預設關閉且 hash-only 的 recovery-code/Passkey replacement flow、observability `0020` schema、pure alert rule/evaluator/parser、尚未接入 Worker entry point/scheduler 的 evaluator run/runtime、alert lifecycle snapshot/CAS 與 bounded audit/OAuth/fan-out/logout/runtime-health/Queue-DLQ source repositories、`0022` evaluator run/source/decision proof ledger、canonical manifest serializer、相容 run repository，以及 state 同批 applied/no-state decision proof integration，另有 pure audit-archive record/envelope crypto contract、`0021` archive ledger、`0023` R2 evidence forward migration、尚未接入 Worker/Queue/Cron 的 D1 repository與只接受 injected R2/envelope verifier 的 pure create-only writer，並有使用 `oauth4webapi` 的獨立 test RP。這些 local API 可證明個別受控 D1 transaction 與 bounded writer decision，但 Cron evaluator、九項 source 收集/decision orchestration、Queue/Email/operator wiring、archive binding/KEK adapter/restore 都不存在，不能宣稱完整同輪 execution proof、production observability 或 encrypted archive 已完成。每個 release candidate 都必須重跑本 repository 的 typecheck、workerd suite、production build 與 test RP protocol gate；本地通過不得寫成遠端已部署。
...
- [x] Observability `0020` schema、pure rule/evaluator/parser、尚未接入 Worker entry point/scheduler 的 bounded audit/OAuth/fan-out/logout/runtime-health/Queue-DLQ source repositories、`0022` evaluator run/source/decision proof ledger、canonical manifest、相容 run repository、lifecycle snapshot 與同批 applied/no-state proof、archive record/envelope crypto contract、`0021` ledger、`0023` evidence contract、unwired D1 archive repository 與 pure unwired create-only R2 writer 已加入 local source並具負面/競態測試；所有 observability/archive repository與 writer 仍未接入 Worker/scheduler，fan-out source 不提供 durable delivery/replay，logout source 不改動 delivery/replay，runtime-health source 不排程 evaluator 或 claim/deliver outbox，Queue source 未新增 binding 或 sampling loop，archive source 未新增 binding/KEK adapter/Queue/Cron/runtime wiring。Cron 九項 source/decision orchestration、alert/archive Queue/DLQ、Email/admin delivery、archive fingerprint derivation/KEK custody、writer integration/remote proof、bounded restore/external backup 與完整同輪 execution proof 仍不存在，所以 observability 必須維持 `source_present_unverified`，archive source 也不能把 `encrypted_r2_archive` 從 `dependency_missing` 升級。
- [ ] 完成並獨立 review observability repository/Cron/delivery proof，以及 encrypted R2 archive 的 fingerprint derivation/KEK custody、create-only writer integration與 remote proof、bounded restore、Queue/Cron 與 external-backup contract；兩個 local report 對任何非 `verified` dependency 都必須維持 blocked + nonzero，全部五項到位後才可記錄 synthetic local pass。
