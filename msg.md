...
### D-部署. 新批次已就緒,等你點頭上線(我依「先不要push」壓著沒部署)
這批(sidebar 重構、頭貼上傳、OAuth 檢舉、consent 重設計、安全活動、五個社群登入 + Telegram、/tos /pp /about、文件/wiki)已全部合併回 main、**144 測試全過**、經對抗性資安審查(無 Critical/High,幾個 Medium/Low 已修)。**但我沒有部署到 production**——你說過這批先不要 push,而且 QA 八角色審查是要在定案前跑的。production 現在仍是上一版 `5ae88125`,乾淨未動。
→ **你的決定**:(1) 我可以部署了嗎?部署 = 套 migration 0011(頭貼表)+ 0012(檢舉表)+ 上新 Worker(社群 secret 未設前那些按鈕會自動隱藏,不影響 Google/Passkey)。(2) 還是你要先自己在本地看過/等 QA 跑完再上?

### D-QA. QA 八角色要審查「哪個網站」?
你列的八個角色(陌生訪客「30秒認識你」、大學教授「審備審資料/學習軌跡」、站主本人「讀你的文章學語氣」…)聽起來是在審查你的**個人網站/部落格/作品集**(有文章、about、學習軌跡),而不是 PGID 這個 SSO 服務。PGID 沒有文章或個人簡介。
→ **你的決定**:QA 八角色要跑在哪個網站?是 PGID(sso.pg72.tw)、你的個人站(哪一個網址/資料夾?)、還是全部?確認後我就派八個角色 agent 去跑。(其中「QA 工程師/美術/AI 讀者/資安」這幾個對 PGID 也適用,我可以先對 PGID 跑;但「陌生訪客/教授/站主本人」比較像針對個人站。)
