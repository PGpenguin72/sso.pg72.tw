...
截至 2026-07-18，repository source 包含 SSO Worker、React 帳號中心、D1 migrations `0001`–`0022`、Google/Passkey、可選社群登入、OAuth 2.1 Provider、四級角色、邀請/停權/audit/client 管理、ID-token central `sid` contract、durable global logout ledger/outbox/Queue delivery、Mail Path A introspection prerequisite、Passkey client-mutation step-up、Telegram verified-email enrollment boundary、全域 provider-identity 唯一 ownership、Turnstile-backed public-registration intent、版本化法律同意紀錄、公開新帳號 restricted access 與 abuse-response runbook、預設關閉且 hash-only 的 recovery-code/Passkey replacement flow、observability `0020` schema、pure alert rule/evaluator/parser、尚未接入 Worker entry point/scheduler 的 evaluator run/runtime、alert lifecycle snapshot/CAS 與 bounded audit/OAuth/fan-out/logout/runtime-health/Queue-DLQ source repositories、`0022` evaluator run/source/decision proof ledger、canonical manifest serializer、相容 run repository，以及 state 同批 applied/no-state decision proof integration，另有 pure audit-archive record/envelope crypto contract、`0021` archive ledger 與尚未接入 Worker/Queue/Cron 的 D1 repository，並有使用 `oauth4webapi` 的獨立 test RP。這些 local API 可證明個別受控 D1 transaction，但 Cron evaluator、九項 source 收集/decision orchestration、Queue/Email/operator wiring 都不存在，不能宣稱完整同輪 execution proof 或 production observability 已完成。每個 release candidate 都必須重跑本 repository 的 typecheck、workerd suite、production build 與 test RP protocol gate；本地通過不得寫成遠端已部署。
...
Local source 也包含 pure audit-archive record/envelope crypto contract：canonical record 保存 nullable actor reference/version，且 header、manifest 與 AES-GCM AAD 綁定 `checkpointFromSequence`。`0021_audit_archive.sql` 提供 monotonic source ledger、immutable batch membership、archive-key sentinel schema、同 lease 的 bounded renewal，以及 trigger-coupled D1 finalization/checkpoint/BLOB cleanup；尚未接線的 request-scoped D1 repository 只接受預先派生的 fingerprint reference，並負責 bounded source prefix、canonical item/parent persistence、claim/renew/terminal receipt、response-loss idempotency 與 audited dead replay。這仍不是 encrypted R2 archive implementation。Archive-specific fingerprint derivation、KEK custody、R2 create-only writer/bounded restore、Queue/DLQ、Cron redrive、retention exercise 與 Cloudflare 之外的 backup 仍未實作，所以 `encrypted_r2_archive` 必須維持 `dependency_missing`。完整 source boundary 見 [`docs/runbooks/alert-observability.md`](./docs/runbooks/alert-observability.md) 與 [`docs/runbooks/audit-archive.md`](./docs/runbooks/audit-archive.md)。
