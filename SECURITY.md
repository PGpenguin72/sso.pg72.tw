...
- Request-scoped guards re-read D1 before provider linking and every admin/developer/client-management permission check, so a stale session cannot retain sensitive PGID access. Existing linked providers remain valid ordinary login methods.
