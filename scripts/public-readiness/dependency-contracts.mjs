...
const RELEASE_SECURITY_TESTS = Object.freeze([
  ...
  Object.freeze({ filename: "production-version-upload-adapter.test.mjs", count: 5 }),
  Object.freeze({ filename: "production-version-upload-gate.test.mjs", count: 27 }),
...
const RELEASE_AUTOMATION_FINGERPRINT_FILES = Object.freeze([
  ...
  "scripts/security/production-version-upload-gate.mjs",
  "scripts/security/production-version-upload-gate.test.mjs",
...
const SQL_CONTRACTS = Object.freeze({
  ...
  observability_0020: {
    ...
    markers: [
      ...
      /create index\s+"audit_event_invalid_occurred_at_idx"/i,
      /create trigger\s+"audit_event_occurred_at_insert_guard"/i,
      /create trigger\s+"audit_event_occurred_at_update_guard"/i,
      /create index\s+"oauth_client_report_invalid_created_at_idx"/i,
      /create trigger\s+"oauth_client_report_created_at_insert_guard"/i,
      /create trigger\s+"oauth_client_report_created_at_update_guard"/i,
      ...
      /create table\s+"alert_evaluator_bootstrap"\s*\(\s*"component"\s+text[\s\S]*?"first_success_at"\s+date[\s\S]*?"source_generation"\s+integer[\s\S]*?"source_revision"\s+integer/i,
      /foreign key\s*\(\s*"component"\s*\)\s+references\s+"alert_runtime_status"\s*\(\s*"component"\s*\)\s+on delete restrict/i,
      ...
      /create trigger\s+"alert_evaluator_bootstrap_insert_guard"/i,
      /create trigger\s+"alert_evaluator_bootstrap_update_guard"/i,
      /create trigger\s+"alert_evaluator_bootstrap_delete_guard"/i,
...
const ARCHIVE_SCHEMA_CONTRACT = Object.freeze({
  ...
  markers: [
    ...
    /create trigger\s+"audit_archive_source_parent_time_guard"/i,
