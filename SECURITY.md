...
The repository's local source now includes the narrowly scoped Mail Path A introspection prerequisite, Passkey step-up for every OAuth client mutation, public-registration prerequisites using Turnstile, versioned legal acceptance and persistent restricted-account access, the global-logout source contract, recovery migration `0019`, observability migration `0020`, pure alert rule/evaluator/parser modules, evaluator runtime-status/lease/bootstrap and alert-state/incident/outbox compare-and-swap repositories, bounded source slices for ten `audit_event` rules and the OAuth client-report rule, the pure audit-archive record/envelope crypto contract, and schema-only archive ledger migration `0021`. The runtime repository owns initialization, lease acquisition/renewal, terminal success/failure through exact compare-and-swap, immutable first-success bootstrap creation through `INSERT ... SELECT`, and the exact projection read; the state repository atomically persists one already-pure evaluator decision with revision fencing, incident state, immutable Email outbox snapshot, canonical payload digest, and explicit applied/conflict/duplicate outcomes; the audit repository owns only the bounded audit-rule observations; the OAuth repository owns bounded half-open report cohorts and nullable reporter-provenance evidence. None of these repositories is imported by the Worker entry point or a scheduler. The remaining fan-out/logout/Queue sources, evaluator Cron, dedicated alert or archive Queue/DLQ, Email Service adapter, operator API/UI, archive R2 writer/bounded restore, external backup, and same-run execution proof remain absent. Observability must therefore remain `source_present_unverified`, while `encrypted_r2_archive` remains `dependency_missing`; the `0021` ledger alone does not make encrypted archive storage operational or verified.
