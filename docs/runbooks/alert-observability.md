...
verify that one invalid parent aborts backfill and that a repaired canonical
parent succeeds, plus the `0021` ledger transaction contract. The archive-crypto
and repository suites verify the record/envelope/checkpoint binding, bounded
source and persistence behavior, exact lease/terminal/replay receipts,
response-loss retries, and concurrent losers without R2 or Queue I/O.
