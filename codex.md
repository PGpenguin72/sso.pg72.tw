...
`0013_confidential_client_secret_post.sql` 將既有 confidential client metadata 正規化為 `client_secret_post`；它不旋轉 secret、不改 grant/token。`0014_passkey_step_up.sql` 新增 session step-up timestamp 與短效 challenge table。兩者都不代表 production 已套用；最後一份 deployment record 只確認 production D1 至 `0012`，必須由 owner 在維護窗口重新驗證遠端 ledger 後再依序執行。
...
`0017_restricted_account_access.sql` 新增獨立於 lifecycle status 的 `user.accessLevel`、既有資料 `standard` default/backfill，以及 restricted role/provider-link/client-owner D1 guards。任何含 restricted request guard 的 Worker 都必須在部署前先套用 `0017`；migration 可在 `invite` 模式下先套用且不會改變 production-target 註冊模式。最後一份 production record 尚未套用 `0017` 或部署此 Worker；這是歷史記錄，維護前必須重新驗證。

`0018_global_logout.sql` 新增 client back-channel URI、實際 `(sid, client_id)` visit ledger、durable logout delivery/attempt evidence 與 access-token visit trigger；舊而未被 runtime 使用的 delivery table 會保留為 `logout_delivery_legacy_0018`。任何含 global-logout Worker 的環境都必須先套用 `0018`，並 provision 專用 logout Queue/DLQ。最後一份 production record 尚未套用 `0018`、部署此 Worker、provision 專用 Queue，或完成 production RP receiver 驗收；維護前必須重新驗證。

`0019_recovery_codes.sql` 新增 recovery code set、一次性 code hash、獨立短效 recovery session 與 Passkey registration challenge；既有使用者不會自動取得 recovery code。Migration 不會自行啟用功能，runtime 仍由 `RECOVERY_MODE` 控制。最後一份 production deployment record 只確認至 `0012`，且未套用 `0019`、綁定 recovery limiter、啟用 recovery、完成獨立 review 或執行 lost-device/rollback drill；這些遠端狀態必須重新驗證。

`0020_alert_observability.sql` 只新增 redacted alert state、incident、immutable Email delivery snapshot/attempt 與 runtime-status schema，並加入 nullable persistent audit actor/OAuth reporter HMAC reference、immutable alert-key continuity sentinel，以及 audit/OAuth/logout 的 bounded evaluator indexes。Actor/reporter ref 首次寫入必須同列保有 raw FK；之後 account deletion 才可用既有 `ON DELETE SET NULL` 清除 raw FK 並保留 immutable ref。Migration 本身不執行 backfill、建立 sentinel row、排程 evaluator、傳送 Queue/Email 或提供 operator endpoint。Repository 另有 pure versioned rule definitions、deterministic evaluator、exact repository-projection parser、負責 evaluator runtime 初始化、lease acquire/renew、terminal success/failure exact CAS、immutable first-success `INSERT ... SELECT` bootstrap 與 exact projection read 的 D1 repository、把一個 pure evaluator decision 以 revision CAS 原子寫入 `alert_state`、`security_alert` 與 canonical `alert_outbox` snapshot 的 D1 repository、只涵蓋 registration/restricted/recovery/Passkey/admin 十項 `audit_event` 規則的 bounded source repository、`pgid.oauth.client_report.v1` 的 bounded source repository、`pgid.security.fanout_gap.v1` 的 global bounded source repository、`pgid.logout.delivery_health.v1` 的 bounded source repository、`pgid.alert.runtime_health.v1` 的 global bounded source repository、`pgid.queue.dlq_approximate.v1` 的 bounded source/streak repository，以及 request-scoped audit-archive D1 repository。Audit source 將 ratio/recovery denominator domain 固定在 `1,000,000`，並以 partial index 和每規則 `LIMIT 1001` 讀取仍需持續評估的 tracked identity；OAuth source 以 exact half-open 5/15/60-minute windows 讀取 total/high-risk/reporter evidence，使用分離的 client/reporter HMAC domain，並讓缺失 reporter provenance 的 distinct count 保持 nullable；fan-out source 以 exact half-open one-hour lookback 比對 `audit_event.id` 與 `security_event_delivery.event_id`，只產生 global、strict older-than-five/fifteen-minute aggregate，對非 canonical source timestamp、錯誤 projection 與不一致 cohort fail closed；logout source 以同一 awaited D1 batch 檢查 sparse invalid-timestamp indexes，分開讀取所有 `created_at < asOf` 的 current unresolved/dead 狀態、delivery-created half-open cohorts 與 terminal `lease_expired` attempt-completed cohorts，保留 global 與 `client_hmac` 維度，並在完整 client projection無法重建 global aggregate 時 fail closed；runtime-health source 在同一 awaited D1 batch 直接重用 immutable bootstrap/runtime projection parser，並以既有 `alert_outbox_due_idx` 讀取所有 current dead row、所有已到期 pending/retry 與 lease 已過期 processing row，只有 post-bootstrap 才產生 global observation；Queue source 只接受四個 closed DLQ dimension 的 request-scoped `Queue.metrics()` 結果，以 provider promise 完成後的 injectable clock 作 `sampledAt`/`asOf`，並用 `changes()`-gated D1 compare-and-swap 保存 exact 60-second nonzero streak，且只有 exact metrics、`metric_sampled_at = sampledAt`、`updated_at = sampledAt` 與 null lease pair 才可視為 response-loss replay；archive repository 負責 fingerprint-only sentinel continuity、bounded source selection、canonical batch persistence、exact lease/terminal receipts 與 audited dead replay，pure writer 則只在 injected R2/envelope-verifier boundary 執行 bounded create/readback decision，pure restore verifier 只在 injected R2-read/custody-opener boundary 執行單一 externally expected object 的 bounded verification/open。這些 repository、writer 與 restore verifier 都有 Workerd regression，且尚未接線的 evaluator orchestration 已以單一 Queue completion `asOf`、closed 九來源 proof、fair bounded work allocation、partial suppression、lease/CAS/terminal fencing 和真實 local D1 integration 組合它們；但 evaluator、archive writer 與 restore verifier 都未由 Worker entry point 或 scheduler 匯入。Fan-out source 不建立通用 durable security-event delivery、replayer 或 Queue wiring；logout source 不改變既有 delivery、replay、Queue 或 Cron 保證；runtime-health source 不排程 evaluator、不 claim/deliver outbox，也不建立新 binding；Queue source 未新增 binding、Cron 或 sampling loop；archive repository/writer/restore verifier 也未新增 binding、real KEK/custody adapter、Queue、Cron 或 runtime wiring。現有 scheduled handler 只排程 logout delivery dispatch，沒有 import、invoke 或 await alert/archive orchestration；generated bindings/config、dedicated alert/archive Queue/DLQ、Email Service adapter、admin API/UI、authenticated independently retained manifest provenance、archive fingerprint derivation/KEK custody、writer/restore runtime R2 integration 與 remote proof、isolated restore sink/exercise、external backup/retention 仍未實作。Pure restore verifier 會驗證 externally supplied exact manifest、object identity、metadata、size 與 stored/computed digest，並回傳 detached records；這不補上上述 operational dependencies。Actor/reporter coverage 不完整、sentinel missing/mismatch、fan-out evidence、logout projection、runtime-health projection 不可信，或 Queue provider/D1 result missing、invalid、stale/conflicting 時都必須保持 unknown/blocked，observability dependency 因此只能是 `source_present_unverified`，不能標記 `verified`。

`0022_alert_evaluator_run_proof.sql` 以三張 additive redacted table 固定 logical Cron occurrence、evaluator lease/run lifecycle、closed 九項 source proof 與 source-attributed decision proof；run insert/renewal 與 runtime terminal transition 由 trigger 在同一 SQLite statement 反向耦合，CAS miss 會 rollback，且直接 legacy evaluator acquire/renew/idle/terminal update 在 `0022` 後 fail closed。Seal 必須已有 exact 九項 source row；healthy terminal 必須 sealed、九項 complete、decision count 相符、沒有 suppressed-partial，並晚於所有 proof write。Local `alert-run-repository` 已提供 occurrence acquire/renew、`asOf` bind、closed source proof、manifest seal、suppressed/no-state decision proof 與 terminal success/failure；`alert-state-repository` 已可從同一 D1 snapshot 重建 lifecycle/CAS，並把 applied proof 放在產品 state/incident/outbox `changes()` chain 的同一 batch，no-state proof 則只在 semantic state 不存在時寫入。Run repository mutation 與 state repository 選用 `runProof` 的路徑都以 exact fence/readback 區分 contention、replay 與 response loss；未帶 `runProof` 的既有 local state-only CAS path 不產生 execution proof。此 orchestrator 與相依 module 尚未由 Worker entry point 或既有 scheduled handler 匯入、呼叫或 await，而該 handler 只排程 logout delivery dispatch；也沒有 generated bindings/config 或 Queue/Email/operator wiring。最後一份 production migration record 只到 `0012`，必須重新驗證；不得把 local synthetic run proof 宣稱成已接線、已部署或 production 能力。

同一個 local `0020` source 另為 `audit_event.occurred_at`、`oauth_client_report.created_at`、`logout_delivery.created_at` 與 non-null `logout_delivery_attempt.completed_at` 建立只包含 non-canonical row 的單欄 partial index，並以 `BEFORE INSERT`/timestamp `UPDATE` guard 阻止未來壞資料。Audit、OAuth、fan-out 與 logout repository 都在同一 D1 batch 的 lexical window 前，以 `INDEXED BY` sparse index 的 bounded `EXISTS ... LIMIT 1` 證明全域 timestamp coverage；任何 legacy corruption 或錯誤 projection 都只回 redacted `source_invalid`，不能產生 known zero。正常空 index 只走 covering probe；legacy row 可更新成 canonical UTC millisecond text 後離開 index。`0021` 在 archive capture/backfill 配發 sequence 前另以 parent `audit_event.id` primary-key lookup 重驗 canonical time，未修復 row 會使整個 backfill statement abort，而不是依錯誤字典序固化。這些仍只是 local migration/repository contract；最後一份 production migration record 只到 `0012`，目前 remote ledger 與 observability wiring 必須由授權 operator 重新驗證。

Local source 也包含 pure audit-archive record/envelope crypto contract：canonical record 保存 nullable actor reference/version，且 header、manifest 與 AES-GCM AAD 綁定 `checkpointFromSequence`。`0021_audit_archive.sql` 提供 monotonic source ledger、immutable batch membership、archive-key sentinel schema、同 lease 的 bounded renewal，以及 trigger-coupled D1 finalization/checkpoint/BLOB cleanup；`0023_audit_archive_r2_evidence.sql` 以前向 migration 加入不造證據的 legacy conflict discriminator、current observed conflict evidence，以及成功封存的 immutable object size/stored SHA/readback SHA fence；`0024_audit_archive_r2_evidence_guard.sql` 保留現有 immutable legacy receipt，並拒絕新 terminal current-format R2-version evidence 缺少 observed byte count。尚未接線的 request-scoped D1 repository 只接受預先派生的 fingerprint reference，並負責 bounded source prefix、canonical item/parent persistence、dispatch-generation claim、lease-bound envelope read、bounded due/expired/checkpoint work selection、renew/terminal receipt、response-loss idempotency 與 audited dead replay。Pure writer 擁有固定 30/120/480/900 秒 retry timing 並提供 `nextAttemptAt`，repository 負責驗證與持久化；只有 transient error 或 lease expiry 在第五次耗盡時成為 `dead`，integrity failure、object conflict 與 readback mismatch 維持 `corrupt`。Writer 啟動時可接受 exact still-live current lease；若 persisted lease 已改變（包括 ambiguous renewal result），則只能 adopt exact same-fence、仍 live 且 update/expiry 都嚴格前進的 descendant lease。Pure writer 透過 injected R2 binding surface 與 envelope verifier 執行 create-only PUT、strong-consistent bounded readback、stored/full SHA 與 byte equality、第二次 envelope 驗證和 exact terminal classification，但沒有 entrypoint/config/binding/KEK adapter。Pure non-HTTP one-object restore verifier 在第一個 injected R2 read 前驗證 externally supplied exact v1 manifest 與 deterministic object key，之後驗證 object identity、metadata、size、stored/computed SHA，透過不含 KEK 的 injected custody opener 開啟，並重新 canonicalize、核對及 detach record set。只有 module/writer-owned temporary copies 會被清除；caller、provider 與 R2-owned buffers 永不被修改。這仍不是完整 encrypted R2 archive implementation。Authenticated independently retained manifest provenance、archive-specific fingerprint derivation、real KEK/custody adapter、runtime R2 binding/integration 與 remote proof、isolated restore sink/exercise、Queue/DLQ、Cron redrive、external backup/retention 仍未實作，所以 `encrypted_r2_archive` 必須維持 `dependency_missing`。完整 source boundary 見 [`docs/runbooks/alert-observability.md`](./docs/runbooks/alert-observability.md) 與 [`docs/runbooks/audit-archive.md`](./docs/runbooks/audit-archive.md)。

Restore verifier result 只證明該次 expected manifest、object bytes 與 canonical record set；它不綁定 R2 version/etag，也不證明 retention、Bucket Lock 或 object-history custody，這些仍是獨立 future gates。

目前也沒有 authenticated、independently retained trusted-manifest provenance adapter/evidence source；R2 object custom metadata 不得自行提供 restore authority，此 gate 仍屬 `dependency_missing`。
...
必須回傳零列。若有任何 duplicate，停止 rollout，獨立審查受影響的使用者與 audit evidence；不得自動刪除、重新指派或合併 identity owner。之後先建立 private backup / Time Travel checkpoint，於隔離 Preview 重新驗證目標 ledger 後，才可依數字順序套用尚未套用的 `0013` 至 head `0024`。
...
既有公開部署紀錄曾顯示 `pg72-id` 部署於 `https://sso.pg72.tw`、production D1 套用至 `0012`，且 Copy 與 Link 的 production traffic 已切換至 PGID。這些只是 last-recorded remote evidence，不建立目前 production 現況。Global logout 與 recovery-code path 只在 local source 完成；同一紀錄將 `RECOVERY_MODE` 記為 disabled，且未包含後續 migration、專用 Queue/DLQ、各 RP receiver、外部告警、完整復原演練或其他 §9.2 gate。任何 maintenance operation 前都必須由授權 operator 重新驗證實際遠端版本、設定與 migration 狀態。
...
Committed production-target `REGISTRATION_MODE` 是 `invite`；這是 source configuration，不是 live deployed mode 證明，遠端狀態必須由授權 operator 重新驗證。`public` 程式路徑與 workerd regression tests 已備妥：所有新帳號都必須通過 verified-email enrollment gate，Google 首次登入要求 verified email，Telegram 因不提供 email 而只能登入已明確連結的既有帳號；邀請功能保留，新帳號建立使用獨立 per-IP `REGISTRATION_RATE_LIMITER`，suspended/deleted 使用者仍由 session 建立檢查擋下。Local source 另要求 exact-hostname/action Turnstile 驗證、版本化 Terms/Privacy 明確同意、一次性 intent，以及公開新帳號持久化 `restricted` access。受限帳號保留普通登入、帳號與 OIDC 使用，但 provider linking 與 PGID developer/admin/client management 皆 fail closed。完整安全 gate 尚未完成，未完成項目列於 §9.2；只有 owner 明確核准、部署並驗證設定變更後才算開啟公開註冊。

Repository source 包含 Mail Path A 所需的 PGID introspection prerequisite。它只允許固定的 `pgid-mail-introspect` 查詢簽發給 `pg72-webmail` 的 opaque access token，且 token 必須保有 live central session、`email` scope，以及 active、verified-email user；完整契約見 §10.5。最後一份 production record 未包含這項 deployment、system-client provisioning 或 production 驗證；目前 remote state 必須重新驗證。Dovecot/Postfix/Roundcube 的 VPS cutover 仍須 owner 在場的維護窗口，不得因本地測試通過而直接套用。
...
- Committed production target 採邀請制；最後一份 remote record 也記為 invite，但 live mode 必須由授權 operator 重新驗證。公開註冊程式路徑保留，只有 §9.2 gate 通過且 owner 明確核准部署後才切換。
...
| --- | --- | --- | --- | --- |
| `copy.pg72.tw` | 第一方 | Next.js / NextAuth、6 位數訪客碼、D1 | PGID OIDC client；訪客碼保持獨立 | 歷史紀錄為 Production live，必須重新驗證；PGID delivery 只在 local source，Copy receiver/rollout 待完成 |
| `link.pg72.tw` | 第一方 | Cloudflare Pages / D1，具有 user/admin | BFF OIDC client，保留應用內角色 | 歷史紀錄為 Production live，必須重新驗證；PGID delivery 只在 local source，Link receiver/rollout 待完成 |
...
> Committed production-target 設定：`REGISTRATION_MODE=invite`。這不是 live mode 證明，必須由授權 operator 重新驗證。
> `public` 程式路徑與測試已備妥，但下列 gate 尚未完成；只有 owner 明確核准並部署設定變更後才算開放。
> 本節分開記錄 last-recorded remote behavior 與 local-source public path；兩者都不能代替授權 operator 的 live re-verification。

### 9.1 邀請註冊（committed target；live mode 待重驗）
...
### 9.2 公開註冊路徑與啟用 gate（local source；remote state 待重驗）
...
- [ ] 在隔離 Preview 套用 `0019`、綁定專用 recovery limiter、以 Preview-only 帳號完成 lost-device/concurrency/rollback/multi-RP logout exercise、獨立 source/security review 與 operator runbook 驗收；committed production target 維持 `RECOVERY_MODE=disabled`，直到 owner 另行核准部署與實機 smoke，且任何操作前都要重新驗證 live setting。
...
### 9.5 Recovery codes（local source；committed target disabled）

- `RECOVERY_MODE` 是獨立 runtime switch；`disabled` 時管理與 recovery endpoints 都回 404。Migration `0019` 本身不啟用功能。Committed production target 必須維持 disabled，直到隔離 Preview、獨立 review、runbook drill 與 owner 核准完成；這項 target 不證明 live setting，操作前必須重新驗證。
...
### 10.5 Mail introspection system client（local source；remote state 待重驗）
...
以上程式與 regression coverage 已收入 repository。最後一份 production record 未包含這個 deployment、system-client provisioning 或 mail VPS cutover 驗收；current remote state 必須重新驗證。
...
Local source 已完成所有 user ID token 的 nonempty central `sid`、refresh
...
API，以及 test RP 的 idempotent receiver。最後一份 production record 未包含
`0018`、專用 Queue/DLQ provisioning、經驗收的 RP receiver 或外部告警；current
remote state 必須重新驗證，因此不能宣稱全域登出已在 production 完成。
...
- 管理員操作要求 fresh authentication；高風險 client mutation 另要求同一 D1 session 的 Passkey step-up。Local source 已實作 required UV、exact origin/RP ID、一次性 session/user-bound challenge、counter guard 與 timestamp/audit 寫入；最後一份 production record 未包含 `0014` 或這個 deployment，current remote state 必須重新驗證，且仍須獨立 review 與實機驗證。
- 管理員至少具有兩種獨立復原方式；local recovery-code source 不能取代第二組 Passkey、獨立 review、isolated Preview drill 或 owner-controlled break-glass planning。最後一份 production record 未包含 `0019` 或 enabled recovery，current remote state 必須重新驗證；這仍是 full Production GO gate。
...
Local ordered migration ledger 已至 `0024`。`0020` 提供 fail-closed observability schema，`0022` 提供 proof ledger；repository 另有 pure evaluator/parser、unwired source/run/state repositories 與九來源 orchestration。Archive 邊界包含 crypto contract、`0021` ledger、`0023` evidence contract、保留 legacy receipt 並拒絕新 R2-version evidence 缺 observed bytes 的 `0024` guard、unwired D1 repository、pure unwired create-only writer 與 pure non-HTTP one-object restore verifier。Writer 以 30/120/480/900 秒固定 schedule 供應 `nextAttemptAt`，repository 驗證與持久化；只有 transient error 或 lease expiry 在第五次耗盡時成為 `dead`，integrity failure、object conflict 與 readback mismatch 維持 `corrupt`。啟動可接受 exact still-live current lease；只有 changed lease 或 ambiguous renewal recovery 才要求 same-fence live strict-descendant adoption。Restore verifier 要求 external exact manifest，驗證 object identity/metadata/size/stored-computed digest 並回傳 detached records。現有 scheduled handler 只排程 logout dispatch，沒有 import、invoke 或 await observability/archive orchestration。Authenticated retained manifest provenance、KEK custody、runtime R2 binding/integration、restore sink/exercise、external backup/retention 與 remote proof 仍缺，因此 observability 狀態仍是 `source_present_unverified`，`encrypted_r2_archive` 仍是 `dependency_missing`。詳細邊界見 [`docs/runbooks/alert-observability.md`](./docs/runbooks/alert-observability.md) 與 [`docs/runbooks/audit-archive.md`](./docs/runbooks/audit-archive.md)。

Trusted-manifest provenance adapter/evidence source 仍未實作；上述 pure restore verifier 的 external expected manifest interface 不得被解讀為已有 authenticated authority source。
...
- 最後一份 deployment record 記載 Link 已切換至 PGID，並以 `oauth4webapi` confidential client、`client_secret_post`、Authorization Code、PKCE S256、state、nonce、ID token 與 UserInfo 驗證取代 Worker 內手寫 Google OAuth；current traffic 與 configuration 必須由授權 operator 重新驗證。
...
Repository source 選定的 Mail Path A 使用 Dovecot introspection + XOAUTH2。§10.5 的固定 service-client／target-client 例外已在 repository source 實作及測試；只有具 live session、`email` scope 與 active verified user 的 `pg72-webmail` opaque access token 才會回 verified email，且該 email 只供 Dovecot 對應既有 mailbox。最後一份 production record 未包含這個 deployment、provisioning 或 production 驗證，current remote state 必須重新驗證；mail VPS 設定、service-secret 注入、disable/rotate 演練與完整 rollback/驗收仍待 owner 維護窗口。
...
以下部署狀態只來自最後一次保留的紀錄，不是 live-state assertion；owner
執行任何 production 變更前必須重新驗證目前狀態。

- 最後一次紀錄描述 Google、Passkey、邀請制 SSO core 已部署，且未設定的可選社群 providers 保持關閉。
- 同一紀錄描述 OIDC Provider、JWKS、client 管理、帳號中心、sessions 與基礎 audit 已部署。
...
- 最後一次紀錄描述 Copy 與 Link 的 production traffic 已切換至 PGID，且 Copy 六位數訪客碼保持獨立；目前 traffic 必須重新驗證。
- 同一紀錄描述 Email 只用於一次性 legacy binding，日常 authentication 使用 SSO `sub`；目前行為必須重新驗證。
- 該歷史紀錄描述的是 invite beta，不是完整 Production GO；ID-token `sid`、visited-client ledger、durable back-channel delivery 與 test receiver 已在 local source 完成，仍待 `0018`/Queue/DLQ Preview rollout、各 production RP receiver、外部告警、rollback drill 及單一/全域登出實機驗收。
