...
- [x] Observability `0020` schema、pure rule/evaluator/parser、bounded 十規則 `audit_event` source slice 與 archive record/envelope crypto contract 已加入 local source 並具負面測試；其餘 D1 sources、state/incident CAS runtime、Cron、alert Queue/DLQ、Email/admin delivery 與同輪 execution proof 尚未實作，所以 observability 必須維持 `source_present_unverified`，pure crypto 也不能把 `encrypted_r2_archive` 從 `dependency_missing` 升級。
