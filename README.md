...
3. Owner runs the provider-identity duplicate preflight in [`handoff.md`](./handoff.md), applies the reviewed pending migrations in numeric order (`0013`, `0014`, then `0015`), and deploys the verified Worker source with `PASSKEY_STEP_UP_MAX_AGE_SECONDS=600` plus Rate Limiting namespaces `1004` and `1005` bound as configured.
